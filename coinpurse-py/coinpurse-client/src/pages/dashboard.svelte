<script lang="ts">
    ...
    accounts
      .map(
        (account) =>
          ({
            ...account,
            institution_name: institutionNameById[account.institution_id] ?? "Unknown",
          }) satisfies AccountWithInstitutionName
      )
      .sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0) || a.account_name.localeCompare(b.account_name))
  );
  ...
  async function loadDashboardData() {
    ...
    error = "";
    try {
      const [accountsData, institutionsData, balancesData] = await Promise.all([
  ...
  function openAddAccount() {
    accountFormError = "";
    accountFieldErrors = { account_name: "", institution_id: "", account_type: "", last_4_digits: "" };
    showAddAccount = true;
  }
  ...
  function validateAccountForm(data: {
    ...
    if (!data.account_name.trim()) {
      ...
      accountFieldErrors.account_name = "Name is too long (max 100 characters)";
      isValid = false;
    }
    ...
    if (data.last_4_digits && data.last_4_digits.length > 4) {
      accountFieldErrors.last_4_digits = "Last 4 digits must be 4 characters or less";
      isValid = false;
    }
  ...
  function validateInstitutionForm(name: string): boolean {
    ...
    if (name.length > 100) {
      institutionFieldErrors.name = "Name is too long (max 100 characters)";
      return false;
    }
    return true;
  }
...
<div class="p-8 max-w-[1400px] mx-auto">
  <div class="flex items-start justify-between gap-4 mb-8">
    <div>
      ...
      <p class="text-muted-foreground">Your accounts and latest balances</p>
    </div>
    <div class="flex gap-2">
      <Button variant="outline" onclick={openAddInstitution}>+ Add Institution</Button>
