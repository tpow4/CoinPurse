<script lang="ts">
	...
	let localAccountIds = $state<string[]>([]);
	let localCategoryIds = $state<string[]>([]);
	let localDatePreset = $state<DatePreset>('last30');

	// Sync local state with props
	$effect(() => {
		localAccountIds = selectedAccountIds;
	});
	$effect(() => {
		localCategoryIds = selectedCategoryIds;
	});
	$effect(() => {
	...
	$effect(() => {
		if (localAccountIds.join(',') !== selectedAccountIds.join(',')) {
			onAccountChange(localAccountIds);
		}
	});
	$effect(() => {
		if (localCategoryIds.join(',') !== selectedCategoryIds.join(',')) {
			onCategoryChange(localCategoryIds);
		}
	});
	$effect(() => {
...
<div class="space-y-2 rounded-lg border p-3">
	...
	<div class="flex flex-wrap items-center gap-3">
		...
		<div class="w-52 min-w-40 max-w-full">
			<MultiCombobox
				id="account"
				items={accountItems}
				bind:values={localAccountIds}
				placeholder={m.txn_filter_all_accounts()}
				searchPlaceholder={m.txn_filter_search_accounts()}
				showSelectAll
				showSelectNone
				selectAllLabel={m.txn_filter_select_all()}
				selectNoneLabel={m.txn_filter_select_none()}
			/>
		</div>
