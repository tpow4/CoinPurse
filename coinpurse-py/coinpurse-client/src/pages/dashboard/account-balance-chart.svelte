<script lang="ts">
  ...
  const seriesBalances = $derived.by(() => {
    ...
    for (const bal of balances ?? []) {
      ...
      if (!existing || sortKey(bal) > sortKey(existing)) byDate.set(bal.balance_date, bal);
    }

    const sorted = [...byDate.values()].sort((a, b) => sortKey(a).localeCompare(sortKey(b)));
    const MAX_POINTS = 90;
  ...
  <Chart.Container config={chartConfig} class="min-h-[180px] w-full">
    <AreaChart
      ...
      props={{
        ...
        xAxis: {
          format: (v: Date) => v.toLocaleDateString(undefined, {day:"2-digit", month: "2-digit" }),
        },
        yAxis: {
      ...
      {#snippet tooltip()}
        <Chart.Tooltip
          labelFormatter={(v: Date) =>
            v.toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" })}
          indicator="line"
        />
      {/snippet}
    </AreaChart>
...
{:else}
  <div class="min-h-[180px] w-full flex items-center justify-center text-sm text-muted-foreground bg-muted/30 border border-dashed rounded-md">
