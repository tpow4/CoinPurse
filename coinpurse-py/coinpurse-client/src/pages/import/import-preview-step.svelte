<script lang="ts">
	...
	let { previewResponse, selectedRows, loading, onSelectionChange, onConfirm, onBack }: Props =
		$props();

	// Categorize transactions
	const validTransactions = $derived(
		previewResponse.transactions.filter((t) => !t.is_duplicate && t.validation_errors.length === 0)
	);
	...
	const selectedDuplicateCount = $derived(
		duplicateTransactions.filter((t) => selectedRows.has(t.row_number)).length
	);
...
<div class="space-y-6">
	...
	<Card.Root class="py-4">
		<Card.Content class="flex items-center justify-between px-4 py-0">
			<div>
				...
				<span class="text-muted-foreground"> transactions selected for import</span>
				{#if selectedDuplicateCount > 0}
					<span class="ml-2 text-amber-600">
						(includes {selectedDuplicateCount} duplicate{selectedDuplicateCount !== 1 ? 's' : ''})
					</span>
				{/if}
			</div>
		</Card.Content>
	...
	{#if duplicateTransactions.length > 0}
		...
		>
			<div class="bg-amber-500/10 p-3 text-sm text-amber-700">
				These transactions appear to already exist in the database. You can still select them for
				import if needed.
			</div>
			<PreviewTransactionTable
	...
	{#if errorTransactions.length > 0}
		...
		>
			<div class="bg-red-500/10 p-3 text-sm text-red-700">
				These transactions have validation errors and cannot be imported.
			</div>
			<PreviewTransactionTable
	...
	<div class="flex items-center justify-between pt-4">
		...
		<Button type="button" onclick={onConfirm} disabled={loading || totalSelected === 0}>
			{#if loading}
				Importing...
			{:else}
