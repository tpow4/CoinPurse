<script lang="ts">
	...
	const allSelected = $derived(
		transactions.length > 0 && transactions.every((t) => selectedRows.has(t.row_number))
	);
...
{#if transactions.length === 0}
	<div class="text-muted-foreground p-8 text-center">No transactions in this section.</div>
{:else}
						...
						{#if selectable}
							<Table.Cell>
								<Checkbox
									...
									onCheckedChange={(checked) => toggleRow(tx.row_number, checked === true)}
								/>
							</Table.Cell>
						{/if}
						<Table.Cell class="text-muted-foreground text-sm">{tx.row_number}</Table.Cell>
						<Table.Cell class="text-sm">{formatDate(tx.transaction_date)}</Table.Cell>
						<Table.Cell class="max-w-[300px] truncate" title={tx.description}>
							{tx.description}
						</Table.Cell>
