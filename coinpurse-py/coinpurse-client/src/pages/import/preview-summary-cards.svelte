<script lang="ts">
	...
	import { CheckCircle, AlertTriangle, Copy, FileText } from '@lucide/svelte';
...
<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
	...
	<Card.Root class="py-4">
		<Card.Content class="flex items-center gap-3 px-4 py-0">
			<div class="flex size-10 items-center justify-center rounded-full bg-amber-500/10">
				<Copy class="size-5 text-amber-500" />
			</div>
			<div>
				<p class="text-muted-foreground text-sm">Duplicates</p>
				<p class="text-2xl font-bold text-amber-600">{summary.duplicate_count}</p>
			</div>
		</Card.Content>
	</Card.Root>
	...
	<Card.Root class="py-4">
		<Card.Content class="flex items-center gap-3 px-4 py-0">
			...
			<div>
				...
				<p class="text-2xl font-bold text-red-600">{summary.validation_errors}</p>
			</div>
		</Card.Content>
	</Card.Root>
</div>
