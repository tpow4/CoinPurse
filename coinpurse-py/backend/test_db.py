...
def seed_initial_data(session):
    ...
    categories_list = [
        ...
        "entertainment", "shopping", "healthcare", "income", "bills"
    ]
...
def test_transactions(session, data):
    ...
    transactions = session.query(Transaction)\
        ...
        .all()
    print(f"✓ Found {len(transactions)} transaction(s) for {data['credit_card'].account_name}")
    
    # Query with joins
    result = session.query(Transaction, Account, Category)\
...
def test_balances(session, data):
    ...
    balances = session.query(AccountBalance)\
...
def test_relationships(session, data):
    ...
    chase = session.query(Institution)\
    ...
    credit_card = session.query(Account)\
        .filter(Account.account_name == "Chase Sapphire")\
        .first()
...
def test_queries(session, data):
    ...
    credit_cards = session.query(Account)\
